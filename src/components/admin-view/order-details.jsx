...
import { CalendarCheck, CheckCircle, CheckCircleIcon, Clock, Clock10, CreditCard, Mail, MapPin, Package, PackageIcon, Phone, ShoppingCart, Truck, User, XCircle } from "lucide-react";
...
function AdminOrderDetailsView({ orderDetails }) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'inProcess': return <Package className="w-4 h-4" />;
      case 'inShipping': return <Truck className="w-4 h-4" />;
      case 'delivered': return <CheckCircleIcon className="w-4 h-4" />;
      case 'pending': return <Clock className="w-4 h-4" />;
      default: return <Clock10 className="w-4 h-4" />;
    }
  };
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'inProcess': return 'bg-blue-500 hover:bg-blue-600';
      case 'inShipping': return 'bg-orange-500 hover:bg-orange-600';
  ...
  return (
   <DialogContent className="w-full h-full max-w-none lg:max-w-[1200px] xl:max-w-[1000px] sm:h-auto max-h-[90vh] overflow-hidden sm:rounded-lg p-0 overflow-y-auto">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <Badge className={`${getStatusColor(orderDetails?.orderStatus)} text-white capitalize text-xs sm:text-sm`}>
                      ...
                      <div className="p-3 sm:p-4 rounded-lg">
                        ...
                        <p className="text-lg sm:text-2xl font-bold text-gray-900">
                          Rs. {formatPrice(orderDetails?.pricing?.totalPrice) ?? 0}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
                <CardHeader className="bg-black text-white rounded-t-lg">
                  <CardTitle className="flex items-center gap-2 text-lg sm:text-xl">
                    ...
                    Order Items ({ensureArray(orderDetails?.cartItems)?.length || 0})
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="divide-y divide-gray-100">
                    {ensureArray(orderDetails?.cartItems) && ensureArray(orderDetails?.cartItems)?.length > 0
                      ? ensureArray(orderDetails?.cartItems)?.map((item, index) => (
